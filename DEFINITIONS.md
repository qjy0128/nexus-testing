...
**执行顺序**：
```
阶段 N 完成 → 独立发送交付物（+ 批准请求如需）→ 等待批准（如需）→ 执行阶段 N+1
```
...
| 降级阶梯级别 | 沙箱状态 |
|------------|---------|
| 1. 真实调用（`sandbox-skill-invoke --mode live`） | OpenClaw CLI 可用时，在沙箱中完整调用 Skill |
| 2. 本地真实执行（`sandbox-skill-invoke --mode shim-live`） | 无 CLI 但有本地适配器时，在沙箱中模拟安装并真实调用 Skill |
| 3. 追踪调用（`sandbox-skill-invoke --mode trace`） | 仅解析 `SKILL.md` 追踪决策树和工具调用路径 |
| 4. 沙箱命令执行（`sandbox-exec`） | 支持 `host-logged` 与 `container` 两种后端；前者只适用于可信命令，后者提供容器级隔离 |
| 5. 构造模拟 | 沙箱不可用或能力不足时 |
| 6. 部分执行 | 沙箱超时或资源限制时 |
| 7. 静态分析 | 最后手段 |
