...
| 优先级 | 方案 | 标注 | 适用说明 |
|--------|------|------|---------|
| 1 | 真实调用（sandbox-skill-invoke live 模式） | 无需标注 | Skill 在沙箱中实际安装并通过 OpenClaw CLI 调用，完整调用链 |
| 2 | 追踪调用（sandbox-skill-invoke trace 模式） | 标注「追踪调用」 | 解析 SKILL.md 追踪决策树，不实际执行工具，但验证逻辑路径 |
| 3 | 沙箱命令执行（sandbox-exec） | 标注「沙箱执行」 | 在沙箱中直接执行命令，验证运行时行为 |
| 4 | 构造模拟输入/输出 | 标注「模拟执行」+ 实际构造的输入输出 | 手动构造工具调用和响应 |
| 5 | 部分执行可执行部分 | 标注「部分执行」+ 已执行步骤 | 仅执行可执行的子步骤 |
| 6 | 降级为静态分析（最后手段） | 必须标注「P1 未执行，仅静态分析，无法验证运行时行为」 | 仅读文档分析 |
...
| 降级阶梯级别 | 沙箱状态 |
|------------|---------|
| 1. 真实调用（sandbox-skill-invoke live） | OpenClaw CLI 可用时，在沙箱中完整调用 Skill |
| 2. 追踪调用（sandbox-skill-invoke trace） | CLI 不可用时，解析 SKILL.md 追踪决策树和工具调用路径 |
| 3. 沙箱命令执行（sandbox-exec） | 直接执行命令验证运行时行为 |
| 4. 构造模拟 | 沙箱不可用或能力不足时 |
| 5. 部分执行 | 沙箱超时或资源限制时 |
| 6. 静态分析 | 最后手段 |
...
|------|:-----------:|:-----------:|:------------:|:-----------:|:-----------:|:-----------:|
| TD-01 触发准确性 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 |
| TD-02 参数空间穷举 | ○ 选测 | ★ 必测 | ★ 必测 | ○ 选测 | ★ 必测 | ★ 必测 |
| TD-03 工具链序列验证 | — | ○ 选测 | ★ 必测 | — | ○ 选测 | ★ 必测 |
| TD-04 多轮上下文保持 | — | — | ○ 选测 | ★ 必测 | — | — |
| TD-05 多轮上下文边界 | — | — | — | ★ 必测 | — | — |
| TD-06 模糊意图处理 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 |
| TD-07 能力边界探测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 |
| TD-08 外部服务故障 | — | ★ 必测 | ★ 必测 | ○ 选测 | ○ 选测 | ○ 选测 |
| TD-09 并发调用 | ○ 选测 | ★ 必测 | ★ 必测 | ★ 必测 | ○ 选测 | ★ 必测 |
| TD-10 输出格式跨渠道 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 |
| TD-11 Token 效率 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 |
| TD-12 部分失败恢复 | — | ★ 必测 | ★ 必测 | ○ 选测 | ○ 选测 | ★ 必测 |
| TD-13 权限边界 | — | ○ 选测 | ○ 选测 | — | ○ 选测 | ★ 必测 |
| TD-14 副作用验证 | — | — | ○ 选测 | — | ○ 选测 | ★ 必测 |
| TD-15 意图解释广度 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 | ★ 必测 |
| TD-16 中间状态检查 | — | ○ 选测 | ★ 必测 | ○ 选测 | ○ 选测 | ★ 必测 |

**图例**：★ 必测 = 必须有对应用例覆盖 | ○ 选测 = 建议覆盖，缺失不阻塞 | — = 不适用
...
### 能力地图（Capability Map）规格

> **由阶段一需求解析师生成**，写入 SPEC.md，是后续所有测试阶段的核心引用。

| 字段 | 说明 |
|------|------|
| Cap-ID | 能力唯一编号，格式 `CAP-{NN}`，全局递增 |
| 能力名称 | 该能力的简短描述 |
| 触发条件 | 用户通过什么方式激活该能力 |
| 使用工具 | 该能力调用的工具列表（从 allowed-tools 和源码分析） |
| 参数列表 | 每个工具的参数：名称、类型、范围、默认值、必填/可选 |
| 输出类型 | 文本/文件/图片/结构化数据/混合 |
| 已知边界 | 从 SKILL.md 和源码中提取的已知限制 |
| 能力链依赖 | 该能力是否依赖其他 CAP-ID 的输出（工具链场景） |
...
- TEST-DESIGN.md 中每个用例必须标注 `capability-id`（对应 CAP-ID）和 `test-dimension`（对应 TD-ID）
- 用例评估师（阶段四）必须构建 CAP × TD 覆盖矩阵，验证所有必测维度被覆盖
- 缺陷分析师（阶段六）根据 CAP-ID 定位缺陷影响范围
- 报告整合师（阶段七）按 CAP-ID 维度展示测试覆盖率
