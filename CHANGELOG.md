...
> v0.9.15 及更早版本的变更记录已归档至 archive/changelog-before-v0.9.16.md。
...
> 注：此条目与上方 v0.9.5（2026-04-03）为同版本的连续更新，下方为首批变更，上方为后续安全/测试增强。
...
### v0.9.42 (2026-04-11) **stage gate + runtime status normalization + Flow A provenance**

- Updated `scripts/generate_stage_subagent_plan.py`, `src/nexus_testing/stage_contracts.py`, `scripts/nexus_stage_executor.py`, and `src/nexus_testing/dispatch_payload_schema.py` so stage dispatch now carries `artifactBaseDir`, `requiredArtifactPaths`, and `upstreamOutputsVerified`, and key stages declare explicit `requiredInputs`.
- Updated `scripts/nexus_stage_executor.py` and added `src/nexus_testing/stage_validation.py` so approval gates now perform lightweight artifact validation before returning `await-approval`, blocking empty or structurally incomplete review files.
- Updated `scripts/nexus_runtime_bridge.py` and added `src/nexus_testing/runtime_status.py` so runtime results normalize `blocked` into `blocked-env` / `blocked-dependency` / `blocked-policy` / `blocked-product`, and silent fallback patterns can trigger `takeover-required` instead of being counted as real PASS.
- Updated `scripts/fixtures/mock_role_runtime.py` and `tests/test_nexus_runtime_bridge.py` with a regression for “tool unavailable then silently switched to web_fetch fallback”.
- Updated `scripts/run_flow_a_skill_execution.py`, `scripts/run_flow_a_takeover_execution.py`, `scripts/validate_flow_a_skill_results.py`, `roles/skill-tester.md`, and `tests/test_flow_a_skill_execution.py` so Flow A stage-five outputs now require `skill-results.meta.json` provenance and validator path consistency checks.
### v0.9.43 (2026-04-11) **stalled watchdog + semantic approval validation + runtime status persistence**

- Updated `scripts/nexus_runtime_bridge.py`, `src/nexus_testing/runtime_config_schema.py`, `scripts/generate_runtime_bridge_config.py`, `runtime-config.claude.json`, and `runtime-config.openclaw.json` so runtime specs now support `stallTimeoutSeconds`; the bridge streams stdout/stderr, watches report-dir artifact changes, and marks long no-output runs as `stalled` instead of waiting only for the hard timeout.
- Updated `scripts/nexus_dispatch_runner.py`, `scripts/nexus_runtime_bridge.py`, and `tests/test_nexus_runtime_bridge.py` so role state files now persist `runtimeStatus`, stage run summaries include `runtimeStatusCounts`, and blocked execution is surfaced as `blocked-env` / `blocked-policy` / `stalled` instead of being flattened into a generic failure note.
- Updated `src/nexus_testing/stage_validation.py`, `scripts/nexus_stage_executor.py`, `tests/test_nexus_stage_executor.py`, and `scripts/fixtures/mock_role_runtime.py` so approval gates validate substantive markdown section content instead of only checking headings/non-empty files; placeholder-only reports are now rejected before `await-approval`.
- Updated `src/nexus_testing/role_runtime_prompt.py`, `scripts/nexus_claude_role_runtime.py`, `scripts/nexus_openclaw_role_runtime.py`, `roles/quality-assessor.md`, and `roles/skill-tester.md` so subagents receive a stricter artifact-path contract, can report `consumedArtifactPaths` / `producedArtifactPaths` / `executionMethod`, and Flow A skill execution is checked against the standard runner contract more aggressively.
- Updated `roles/skill-tester.md`, `roles/security-tester.md`, `roles/functional-tester.md`, `roles/compatibility-tester.md`, `roles/performance-tester.md`, `roles/accessibility-auditor.md`, `roles/reality-checker.md`, and `roles/mcp-tester.md` so takeover policy metadata now points at `blocked-env`, `blocked-policy`, and `stalled` instead of a single coarse `blocked` bucket.
