...
WINDOWS_BASH_BLACKLIST = {r"c:\windows\system32\bash.exe"}
...
def find_runnable_bash() -> str:
    ...
    if os.name == "nt":
        candidates.extend(str(path) for path in WINDOWS_GIT_BASH_CANDIDATES)
    ...
    for candidate in candidates:
        ...
        if lowered in seen or lowered in WINDOWS_BASH_BLACKLIST:
            continue
        seen.add(lowered)
