...
def test_run_until_gate_flow_a() -> None:
    ...
    try:
        ...
        delivery_record = report_dir / "DELIVERY" / "FINAL-TEST-REPORT.delivery.json"
        assert_equal(delivery_record.exists(), True, "delivery record exists")
        delivery_payload = json.loads(read_text(delivery_record))
        assert_equal(delivery_payload["receipt"]["backend"], "relay-only", "default auto delivery backend")
        assert_equal(delivery_payload["receipt"]["status"], "prepared", "default auto delivery status")
        assert_equal(final["delivery"]["status"], "prepared", "final delivery status surfaced")
