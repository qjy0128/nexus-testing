...
主 agent 在发送阶段零环境就绪报告时，**必须同时提示 Flow B 预判模式**（来自 `environment-checker` 的 `flowBModePrediction` 字段）：

```text
当前文档完整度预判：{A 模式（8 阶段）/ B 模式（11 阶段）}
最终模式将在阶段二质量评估后确认。
```

此提示的目的是让用户在阶段零确认时就了解总流程长度，避免到阶段二才突然切换模式造成预期落差。
若用户对预判有异议，可在确认时反馈；主 agent 将该反馈传递给阶段二 quality-assessor 参考。
